...
ERA_STICKSANDSTONES = [
    ...
    dict(defName="WindbreakShelters", label="windbreak shelters", track="ConstructionSettlement", prereqs=["Woodworking"],
    ...
    dict(defName="Counting", label="counting", track="KnowledgeWriting", prereqs=["Language"],
         desc="Fingers, pebbles, and notched sticks turn 'some' and 'many' into an exact number."),
    ...
    dict(defName="BoneSetting", label="bone setting", track="MedicineHealth", prereqs=["HerbalRemedies"],
         desc="A splint bound tight and held still lets a broken bone knit instead of crippling for good."),
...
ERA_AGRARIAN = [
    ...
    dict(defName="FoodPreservation", label="food preservation", track="SurvivalFood", prereqs=["Pottery"],
    ...
    dict(defName="Villages", label="villages", track="ConstructionSettlement", prereqs=["Woodworking"],
    ...
    dict(defName="Threshing", label="threshing", track="AgricultureAnimals", prereqs=["Agriculture"],
         desc="Beating cut stalks free of their grain is the last labor between a harvest and a meal."),
    ...
    dict(defName="ChiefdomLeadership", label="chiefdom leadership", track="SocietyGovernance", prereqs=["Villages"],
    ...
    dict(defName="Pictographs", label="pictographs", track="KnowledgeWriting", prereqs=["Storytelling"],
         desc="Drawn symbols stand for a herd, a harvest, or a name, kept without yet being full writing."),
    dict(defName="AnimisticReligion", label="animistic religion", track="BeliefCulture", prereqs=["Language"],
    ...
    dict(defName="Barter", label="barter", track="TradeEconomy", prereqs=["Language"],
...
ERA_BRONZE = [
    ...
    dict(defName="Causeways", label="causeways", track="TransportExploration", prereqs=["Cart"],
         desc="A raised, packed-earth track lets a cart cross marsh and ford a shallow river without bogging down."),
    ...
    dict(defName="Kingship", label="kingship", track="SocietyGovernance", prereqs=["UrbanPlanning"],
         desc="One ruling house, its claim inherited rather than earned, over a city too large to govern by council."),
    dict(defName="TributeSystems", label="tribute systems", track="SocietyGovernance", prereqs=["Kingship"],
         desc="A king's warband collects tribute from villages under his claim, whether they consent or not."),
    ...
    dict(defName="Priesthood", label="priesthood", track="BeliefCulture", prereqs=["Kingship"],
         desc="A dedicated class of ritual specialists, housed in temples a king helps pay for."),
    dict(defName="SacredSites", label="sacred sites", track="BeliefCulture", prereqs=["Priesthood"],
         desc="Groves and springs set aside as sacred, watched over by priesthood, need no walls to be holy."),
    dict(defName="Mythology", label="mythology", track="BeliefCulture", prereqs=["Writing"],
    ...
    dict(defName="LongDistanceTrade", label="long-distance trade", track="TradeEconomy", prereqs=["Sailing"],
    ...
    dict(defName="Midwifery", label="midwifery", track="MedicineHealth", prereqs=["HerbalRemedies"],
...
ERA_CLASSICAL = [
    dict(defName="Aquaculture", label="aquaculture", track="SurvivalFood", prereqs=["Irrigation"],
    ...
    dict(defName="Sanitation", label="sanitation", track="ConstructionSettlement", prereqs=["Masonry", "Irrigation"],
    ...
    dict(defName="Astronomy", label="astronomy", track="KnowledgeWriting", prereqs=["Mathematics"],
    ...
    dict(defName="Cartography", label="cartography", track="KnowledgeWriting", prereqs=["Geometry"],
         desc="A map drawn to scale turns a captain's memory of a coastline into something another captain can read."),
    ...
    dict(defName="OrganizedReligion", label="organized religion", track="BeliefCulture", prereqs=["Mythology"],
    ...
    dict(defName="SiegeEngines", label="siege engines", track="Warfare", prereqs=["MonumentalArchitecture"],
         desc="Torsion and counterweight turn stored energy into a wall-breaking blow."),
    dict(defName="StandingArmies", label="standing armies", track="Warfare", prereqs=["Bureaucracy"],
         desc="Soldiers paid and drilled year-round, not called up from the fields each campaign season."),
    dict(defName="TradeRoutes", label="trade routes", track="TradeEconomy", prereqs=["Roadbuilding"],
    ...
    dict(defName="Navigation", label="navigation", track="TransportExploration", prereqs=["Sailing"],
         desc="Star sightings and dead reckoning let a captain find a coast without ever seeing it first."),
    dict(defName="Caravans", label="caravans", track="TransportExploration", prereqs=["Cart"],
...
ERA_MEDIEVAL = [
    ...
    dict(defName="Confectionery", label="confectionery", track="SurvivalFood", prereqs=["Winemaking"],
    ...
    dict(defName="Castles", label="castles", track="ConstructionSettlement", prereqs=["Steel"],
    ...
    dict(defName="HeavyPlow", label="heavy plow", track="AgricultureAnimals", prereqs=["Steel"],
    ...
    dict(defName="Haymaking", label="haymaking", track="AgricultureAnimals", prereqs=["AnimalHusbandry"],
         desc="Cut and cured before the frost, a stored store of hay carries a herd through a winter that killed the grass months before."),
    dict(defName="Feudalism", label="feudalism", track="SocietyGovernance", prereqs=["Kingship"],
         desc="Land granted in exchange for loyalty and arms lets a king rule through his own nobles."),
    dict(defName="GuildSystem", label="guild system", track="SocietyGovernance", prereqs=["GuildHalls"],
    ...
    dict(defName="Algebra", label="algebra", track="KnowledgeWriting", prereqs=["Mathematics"],
    ...
    dict(defName="Monasticism", label="monasticism", track="BeliefCulture", prereqs=["Priesthood", "Mythology"],
    ...
    dict(defName="Knighthood", label="knighthood", track="Warfare", prereqs=["SteelWeapons"],
    ...
    dict(defName="GuildTrade", label="guild trade", track="TradeEconomy", prereqs=["GuildSystem"],
    ...
    dict(defName="Windmills", label="windmills", track="EnergyIndustry", prereqs=["Wheel", "Masonry"],
         desc="Sails on a turning cap catch wind a river's current never offered, and grind grain anywhere it blows."),
    dict(defName="Compass", label="compass", track="TransportExploration", prereqs=["Sailing", "Astronomy"],
         desc="A floating lodestone always points north, in fog or open ocean, when the stars are hidden."),
    dict(defName="OceanicNavigation", label="oceanic navigation", track="TransportExploration", prereqs=["Navigation"],
         desc="Compass, chart, and reckoning together let a captain leave sight of any coast at all."),
    dict(defName="Shipbuilding", label="shipbuilding", track="TransportExploration", prereqs=["Steel"],
         desc="Steel-fastened, multi-decked hulls survive ocean crossings a coastal galley never could."),
]

ERA_INDUSTRIAL = [
    dict(defName="Canning", label="canning", track="SurvivalFood", prereqs=["TinPlating"],
         desc="Food sealed airtight in a tin-plated can outlasts salting or smoking by years, not seasons."),
    dict(defName="FoodRefrigeration", label="food refrigeration", track="SurvivalFood", prereqs=["Electricity"],
    ...
    dict(defName="SyntheticFertilizer", label="synthetic fertilizer", track="AgricultureAnimals", prereqs=["Thermodynamics"],
    ...
    dict(defName="Printing", label="printing", track="KnowledgeWriting", prereqs=["MetalCasting"],
         desc="Cast metal type, reused page after page, turns one copied book into a thousand identical ones."),
    dict(defName="Journalism", label="journalism", track="KnowledgeWriting", prereqs=["Printing"],
         desc="A press run on a fixed schedule turns news into something bought daily, not gossiped once."),
    dict(defName="ScientificMethod", label="scientific method", track="KnowledgeWriting", prereqs=["Printing"],
    ...
    dict(defName="Humanism", label="humanism", track="BeliefCulture", prereqs=["ScientificMethod"],
         desc="Human reason and dignity, not divine decree alone, become a legitimate ground for belief."),
    dict(defName="Firearms", label="firearms", track="Warfare", prereqs=["BessemerProcess"],
    ...
    dict(defName="StockExchanges", label="stock exchanges", track="TradeEconomy", prereqs=["Banking"],
    ...
    dict(defName="Anesthesia", label="anesthesia", track="MedicineHealth", prereqs=["Anatomy", "Pharmacology"],
    ...
    dict(defName="CoalMining", label="coal mining", track="EnergyIndustry", prereqs=["Pottery"],
         desc="Deep-shaft coal mining supplies a fuel denser and more abundant than any forest could regrow."),
    dict(defName="SteamEngine", label="steam engine", track="EnergyIndustry", prereqs=["CokeSmelting"],
    ...
    dict(defName="SteamShips", label="steam ships", track="TransportExploration", prereqs=["SteamEngine"],
         desc="A steam-driven hull keeps schedule against wind and tide, where a sailing ship only guesses at both."),
    dict(defName="BooleanLogic", label="boolean logic", track="InformationComputation", prereqs=["Geometry"],
...
ERA_INFORMATION = [
    dict(defName="FoodScience", label="food science", track="SurvivalFood", prereqs=["Biochemistry"],
         desc="Understanding food at the level of chemistry, not just heat and salt, engineers what keeps and what spoils."),
    dict(defName="VerticalFarming", label="vertical farming", track="SurvivalFood", prereqs=["Computers"],
         desc="Stacked, computer-tended growing trays put a field's yield inside a single climate-controlled building."),
    dict(defName="Plastics", label="plastics", track="MaterialsCrafts", prereqs=["Petrochemistry"],
    ...
    dict(defName="CompositeMaterials", label="composite materials", track="MaterialsCrafts", prereqs=["Alloys"],
    ...
    dict(defName="Megacities", label="megacities", track="ConstructionSettlement", prereqs=["Skyscrapers"],
         desc="Zoned districts of towering density let a single city hold a population once spread across a nation."),
    dict(defName="Suburbs", label="suburbs", track="ConstructionSettlement", prereqs=["AutomobileEngineering"],
         desc="A car in every driveway lets a city spread outward instead of only upward."),
    dict(defName="Hydroponics", label="hydroponics", track="AgricultureAnimals", prereqs=["ThreeFieldSystem", "Thermodynamics"],
    ...
    dict(defName="Democracy", label="democracy", track="SocietyGovernance", prereqs=["Citizenship", "NationState"],
         desc="Universal suffrage extends the constitution's promise from a propertied few to every citizen."),
    dict(defName="WelfareState", label="welfare state", track="SocietyGovernance", prereqs=["Democracy"],
    ...
    dict(defName="PublicLibraries", label="public libraries", track="KnowledgeWriting", prereqs=["Encyclopedism"],
         desc="A library open to every citizen, not just scholars, makes a printed encyclopedia's knowledge common property."),
    dict(defName="SecularEthics", label="secular ethics", track="BeliefCulture", prereqs=["Democracy"],
         desc="A framework of right and wrong argued from reason and consent, not scripture, guides a citizen who votes."),
    dict(defName="MassMedia", label="mass media", track="BeliefCulture", prereqs=["Electricity"],
    ...
    dict(defName="Computers", label="computers", track="InformationComputation", prereqs=["Electricity"],
         desc="Electrically switched logic gates carry out boolean reasoning faster than any hand ever could."),
    dict(defName="VideoGames", label="video games", track="InformationComputation", prereqs=["Computers"],
         desc="A computer repurposed for play turns idle processing power into an industry of its own."),
    ...
    dict(defName="ComputerGraphics", label="computer graphics", track="InformationComputation", prereqs=["Semiconductors"],
         desc="Pixels driven by silicon render a picture instead of only a number, and a screen becomes a canvas."),
    dict(defName="Telecommunications", label="telecommunications", track="InformationComputation", prereqs=["Telegraphy"],
    ...
    dict(defName="AsteroidMining", label="asteroid mining", track="FrontierExotic", prereqs=["SpaceRocketry"],
...
ERA_EXOTIC = [
    ...
    dict(defName="Xenobotany", label="xenobotany", track="AgricultureAnimals", prereqs=["SelectiveBreeding", "Biochemistry"],
         desc="Genetic tools built for earthly crops get turned on organisms suited to soils and skies not our own."),
    dict(defName="AIAdministration", label="AI administration", track="SocietyGovernance", prereqs=["Computers"],
         desc="Automated systems apply policy at a scale and consistency no human civil service ever matched."),
    dict(defName="MemeticCulture", label="memetic culture", track="BeliefCulture", prereqs=["SecularEthics", "Electricity"],
    ...
    dict(defName="LifeExtension", label="life extension", track="MedicineHealth", prereqs=["Biochemistry"],
    ...
    dict(defName="OrbitalRingConstruction", label="orbital ring construction", track="FrontierExotic", prereqs=["AsteroidMining"],
         desc="A structure encircling the whole planet, built from mined asteroid material, replaces rockets with an elevator."),
    dict(defName="TerraformingEngineering", label="terraforming engineering", track="FrontierExotic", prereqs=["AsteroidMining"],
         desc="Engineered atmospheres and seeded xenobotany work together to make a dead world breathable."),
    dict(defName="WormholeTheory", label="wormhole theory", track="FrontierExotic", prereqs=["Semiconductors", "ArtificialIntelligence"],
    ...
    dict(defName="MindUploading", label="mind uploading", track="FrontierExotic", prereqs=["NeuralLace"],
