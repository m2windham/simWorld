...
fill; a `PreceptDef` is the specific rule, reaching mood through the *existing*
...
*whether* a citizen currently upholds the rule — the base class is
...
needing a new event hook into another module. One worker class serves *both*
...
dictionary, Scribe'd by reference) — deliberately a *second* concept from
...
table could not be sourced from this environment, so only the *shape* (several
