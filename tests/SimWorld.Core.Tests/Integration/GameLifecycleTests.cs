...
namespace SimWorld.Tests.Integration
{
    ...
    public class GameLifecycleTests : ContentTestBase
    {
        ...
        public void Scribe_round_trip_of_a_whole_game_preserves_world_settlement_population_and_time_and_keeps_ticking()
        {
            ...
            // is also what re-registers this very loaded game's off-map citizens — the tick lists are rebuilt
            // from the maps alone, so nobody standing on none of them comes back on one), after the
            // map-to-ledger seam and the industry it feeds made it 16, the works initiative made it 14 and the
            // stonework initiative made it 13.
            Assert.Equal(17, loaded.TickManager.PostTickers.Count);
