...
namespace SimWorld.Quests
{
    ...
    /// <see cref="DifficultyDef.questRewardValueFactor"/> — dormant until this class existed, since nothing
    /// paid a reward for it to scale — multiplies the silver, because it is a factor on reward *value* and
    /// silver is the only value here. Goodwill is deliberately left alone: it is a diplomatic reading clamped
    /// to [-100, 100] whose thresholds decide war and alliance
    ...
    /// of silver is bigger".
    ///
    /// <para/><b>A reward is never rounded away.</b> A positive award under a positive factor always pays at
    /// least one, the same "no engagement is ever free" floor <see cref="SettlementRaidResolver"/> applies to
    /// casualties, so a small reward on a low difficulty cannot silently become the bug this class was
    /// written to remove.
    ///
    ...
    public sealed class QuestRewardSink : IQuestRewardSink
    {
        ...
        /// <summary>The difficulty's reward-value multiplier, floored at zero (a negative factor is not a
        /// smaller reward, it is a nonsense one). 1 while no difficulty is set.</summary>
        public static float ValueFactor
        {
            get
            {
                DifficultyDef? difficulty = Find.Storyteller.difficulty;
                float factor = difficulty?.questRewardValueFactor ?? 1f;
