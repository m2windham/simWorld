...
namespace SimWorld.God
{
    ...
    /// this class owns no registry of its own. <c>World.Settlement</c> — which landed alongside this module —
    /// is what should feed <see cref="Recompute"/>: it already holds population by tier, with real
    /// <c>Pawn</c> objects only above Statistical and that tier as a bare count. Wiring the two together is
    /// deliberately not done here (see <c>docs/status.json</c>, <c>god.settlement-population</c>), because it
    /// needs a decision this module should not make alone: a settlement's Statistical citizens have no
    /// <c>Pawn</c> to sample at all, so the rollup has to learn to fold in a cohort with no members to walk.
    /// Until then a caller passes the living population directly, the same contract
    /// <c>FamilyManager.DemographyTick</c> already uses for its own population-wide sweep.
    /// <para/>
